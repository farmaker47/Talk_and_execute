...
void encode(Tokenizer *t, char *text, int *tokens, int *n_tokens) {
    ...
    char *str_buffer = malloc((t->max_token_length * 2 + 1 + 2) *
                              sizeof(char)); // *2 for concat, +1 for null terminator +2 for UTF8 (in case max_token_lenght is 1)
    size_t str_len = 0;
...
int run_inference(JNIEnv *env, jobject thiz, char *checkpoint_path, char *tokenizer_path,
    ...
    while (pos < steps) {
        ...
        on_new_token_callback(env, thiz, piece);
        token = next;
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jclass clazz = (*env)->NewGlobalRef(env, (*env)->FindClass(env,
                                                               "com/example/talkandexecute/llm/InferenceRunner"));
